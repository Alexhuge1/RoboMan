...
class hipnuc_parser:
    ...
    def _parse_hi92(self, data, ofs):
        ...
        self.frame.gyr = [
            x * 0.001 * R2D for x in struct.unpack_from("<3h", data, ofs + 10)
        ]
        self.frame.acc = [
            x * 0.0048828 / GRAVITY for x in struct.unpack_from("<3h", data, ofs + 16)
        ]
        self.frame.mag = [
            x * 0.030517 for x in struct.unpack_from("<3h", data, ofs + 22)
        ]
        ...
        self.frame.quat = [
            x * 0.0001 for x in struct.unpack_from("<4h", data, ofs + 40)
        ]
    ...
    def print_parsed_data(data):
        ...
        if data.frame_type is not None:
            data_fields = [
                ...
                (
                    "Sync_time (s)",
                    f"{data.sync_time:<9.3f}" if data.sync_time is not None else None,
                ),
...
if __name__ == "__main__":
    ...
    example_data_hi91 = bytes.fromhex(
    ...
    example_data_hi92 = bytes.fromhex(
        "5A A5 30 00 E3 A4 92 23 23 1C 7C 11 61 FF 23 23 00 00 01 00 00 00 EB FE 7B 00 B6 07 CD 03 70 FC A3 FC 56 1F 00 00 CF 0D 00 00 C4 07 00 00 F0 26 27 01 C0 02 C2 00 "
    )
    example_data_hi81 = bytes.fromhex(
        "5A A5 68 00 70 04 81 00 02 01 15 09 43 C8 3E 02 00 00 83 00 3C 00 36 01 8F FB E8 04 31 04 7C 02 B2 F9 BF FE 60 79 00 10 24 18 07 1C 0A 1B EB 74 9A 0E 8A 0E 2E 18 20 20 3B 10 19 04 65 F1 99 6A 5D 45 31 61 CE 17 06 D1 00 00 0C 07 01 1C 00 00 00 B9 FC 01 00 00 02 00 00 00 C6 FF 08 01 E7 FF 7F 12 41 00 4E 61 CE 17 04 CF 00 00 00 00"
    )
