...
def send_command_and_wait_for_response(
    ser, command_str, timeout=0.2, ignore_non_text=False
):
    """
    ...
    if ignore_non_text:
        ...
        response = b"".join(
            [bytes([b]) for b in response if 32 <= b <= 126 or b in (10, 13)]
        )
...
@click.option(
    ...
    "-p",
    required=True,
    help="The serial port to connect to (e.g., COM3 or /dev/ttyUSB0)",
)
@click.option(
    "--baudrate",
    "-b",
...
def cmd_send(port, baudrate, command):
    ...
    try:
        ...
        for attempt in range(max_retries):
            success, response = send_command_and_wait_for_response(
                ser, "AT+EOUT=0", ignore_non_text=True
            )
            if success:
                break
        ...
        success, response = send_command_and_wait_for_response(
            ser, command, ignore_non_text=False
        )
        if not success:
        ...
        success, response = send_command_and_wait_for_response(
            ser, "SAVECONFIG", ignore_non_text=True
        )
        if not success:
        ...
        success, response = send_command_and_wait_for_response(
            ser, "AT+EOUT=1", ignore_non_text=True
        )
        if not success:
