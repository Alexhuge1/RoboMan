...
def reveice_process(data):
    ...
    if data[0] == 100 and data[1] == 99:
        ...
        for i in range(6):
            temp_value = (np.uint16(receive_data[i][1]) << 8) | np.uint16(
                receive_data[i][2]
            )
            CurPosition = uint_to_float(temp_value, P_MIN, P_MAX, 16)
            temp_value1 = (np.uint16(receive_data[i][3]) << 4) | (
                (receive_data[i][4] & 0xF0) >> 4
            )
            CurVelocity = uint_to_float(temp_value1, V_MIN, V_MAX, 12)
            temp_value2 = ((np.uint16(receive_data[i][4]) & 0x000F) << 8) | np.uint16(
                receive_data[i][5]
            )
            CurTorque = 0.28 * uint_to_float(temp_value2, A_MIN, A_MAX, 12)
...
def CanComm_ControlCmd(cmd, GM8115_ID):
    ...
    buf = np.array(
        [GM8115_ID, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0x00], np.uint8
    )
